...
from src.subscriptions.models import SubscriptionPlan, CustomerSubscription, BillingHistory
...
async def create_seed_data():
    ...
    async with async_session() as session:
        ...
        organization_members = [
            ...
            OrganizationMember(organization_id=organizations[0].id, user_id=users[3].id, role="member"),
            OrganizationMember(organization_id=organizations[0].id, user_id=users[7].id, role="member"),
            
            # Marketing Team (4 members)
            OrganizationMember(organization_id=organizations[1].id, user_id=users[1].id, role="owner"),
            OrganizationMember(organization_id=organizations[1].id, user_id=users[2].id, role="admin"),
            OrganizationMember(organization_id=organizations[1].id, user_id=users[4].id, role="member"),
            OrganizationMember(organization_id=organizations[1].id, user_id=users[5].id, role="member"),
            
            # Research Team (3 members)
            OrganizationMember(organization_id=organizations[2].id, user_id=users[8].id, role="owner"),
            OrganizationMember(organization_id=organizations[2].id, user_id=users[3].id, role="admin"),
            OrganizationMember(organization_id=organizations[2].id, user_id=users[7].id, role="member"),
            
            # Sales Department (3 members)
            OrganizationMember(organization_id=organizations[3].id, user_id=users[2].id, role="owner"),
            OrganizationMember(organization_id=organizations[3].id, user_id=users[3].id, role="member"),
            OrganizationMember(organization_id=organizations[3].id, user_id=users[7].id, role="member"),
            
            # Customer Success (2 members)
            OrganizationMember(organization_id=organizations[4].id, user_id=users[0].id, role="owner"),
            OrganizationMember(organization_id=organizations[4].id, user_id=users[1].id, role="admin"),
        ]
        ...
        for member in organization_members[5:10]:  # Some members invited later
            activities.append(ActivityLog(
                ...
                action_type="organization",
                description=f"User invited to organization",
        ...
        for billing in billing_records:
            payment_activities.append(ActivityLog(
                ...
                organization_id=subscriptions[0].organization_id if billing.subscription_id == sub1.id else 
                               subscriptions[1].organization_id if billing.subscription_id == sub2.id else
                               subscriptions[2].organization_id,
                ip_address=random.choice(IP_ADDRESSES),
                user_agent=random.choice(USER_AGENTS),
        ...
        print(f"     - {sum(1 for u in users if u.status == 'active')} active users")
        print(f"     - {sum(1 for u in users if u.status == 'invited')} invited users")
        print(f"     - {sum(1 for u in users if u.status == 'suspended')} suspended users")
        print(f"     - {sum(1 for u in users if u.is_verified)} verified users")
        print(f"   • {len(organizations)} organizations with {len(organization_members)} total memberships")
        print(f"   • {len(projects)} projects")
        print(f"   • {len(activities) + len(payment_activities)} activity logs")
        print(f"   • {len(subscriptions)} subscriptions ({active_subscriptions} active)")
        print(f"   • {len(billing_records)} billing records (Total revenue: ${total_revenue / 100:.2f})")
        print(f"\n🔐 Default password for all users: {admin_password}")
        print(f"\n� User accounts:")
        print(f"   • admin@example.com (admin, superuser)")
        print(f"   • john@example.com (member, active)")
        print(f"   • jane@example.com (member, active)")
        print(f"   • sarah@example.com (member, active)")
