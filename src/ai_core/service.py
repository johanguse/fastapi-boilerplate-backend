...
class AIService:
    ...
    async def generate_embeddings(
        ...
        try:
            # Count tokens
            total_tokens = sum(
                self.provider.count_tokens(text) for text in texts
            )
            ...
            logger.info(
                f'AI embeddings generated: {len(texts)} texts, {total_tokens} tokens'
            )
...
Answer:"""
    ...
    async def generate_content(
        ...
        prompt = prompts.get(
            content_type, f'Write {tone} content about {topic}'
        )
