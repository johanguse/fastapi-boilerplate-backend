...
class AIDashboardService:
    ...
    async def get_dashboard_overview(
        ...
        try:
            ...
            monthly_credits = (
                subscription.plan.max_ai_credits_monthly
                if subscription and subscription.plan
                else 0
            )
            ai_features = (
                subscription.plan.ai_features_enabled
                if subscription and subscription.plan
                else []
            )
            ...
            content_generations_count = (
                await self._get_content_generations_count(organization_id)
            )
            analytics_queries_count = await self._get_analytics_queries_count(
                organization_id
            )
            ...
            daily_avg = (
                (week_usage.get('total_tokens', 0) / 7)
                if week_usage.get('total_tokens', 0) > 0
                else 0
            )
            ...
            return {
                ...
                'usage': {
                    ...
                    'usage_percentage': round(
                        (credits_used / monthly_credits * 100), 2
                    )
    ...
    async def get_usage_trends(
        ...
        try:
            ...
            mid_point = days // 2
            first_half_tokens = sum(
                day['tokens'] for day in daily_usage[:mid_point]
            )
            second_half_tokens = sum(
                day['tokens'] for day in daily_usage[mid_point:]
            )
            ...
            if first_half_tokens > 0:
                growth_rate = (
                    (second_half_tokens - first_half_tokens)
                    / first_half_tokens
                ) * 100
    ...
    async def get_feature_insights(
        ...
        try:
            ...
            usage_by_feature = await self._get_usage_by_feature(
                organization_id, month_start, current_month
            )
                ...
                if stats['tokens'] > 0:
                    feature_efficiency[feature] = {
                        'cost_per_token': round(
                            stats['cost'] / stats['tokens'], 6
                        ),
                        'tokens_per_operation': round(
                            stats['tokens'] / stats['operations'], 2
                        )
            ...
            recommendations = self._generate_recommendations(
                usage_by_feature, features_sorted
            )
            ...
            return {
                ...
                'most_used_features': [
                    {'feature': f, 'tokens': t} for f, t in features_sorted[:3]
                ],
    ...
    async def _get_usage_by_feature(
        ...
        result = await self.db.execute(
            select(
                ...
                func.sum(AIUsageLog.cost).label('cost'),
                func.count(AIUsageLog.id).label('operations'),
                func.avg(AIUsageLog.tokens_used).label(
        ...
        for row in result:
            usage_by_feature[row.feature] = {
                ...
                'avg_tokens_per_operation': round(
                    float(row.avg_tokens_per_operation or 0), 2
                ),
            }
    ...
    async def _get_content_generations_count(
        self, organization_id: int
    ) -> int:
        """Get count of AI content generations for organization."""
        result = await self.db.execute(
            select(func.count(AIContentGeneration.id)).where(
                AIContentGeneration.organization_id == organization_id
            )
        )
        return result.scalar() or 0
    ...
    async def _get_analytics_queries_count(self, organization_id: int) -> int:
        ...
        result = await self.db.execute(
            select(func.count(AIAnalyticsQuery.id)).where(
    ...
    def _generate_recommendations(
        ...
        if not usage_by_feature:
            recommendations.append(
                'Start using AI features to see insights and recommendations.'
            )
