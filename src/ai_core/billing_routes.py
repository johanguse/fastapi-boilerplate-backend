...
async def get_billing_period(
    ...
    try:
        ...
        service = AIBillingService(db)
        bill = await service.calculate_usage_bill(
        ...
        raise HTTPException(
            status_code=500, detail='Failed to get billing period'
        )
...
async def check_usage_limits(
    ...
    estimated_tokens: int = Query(
        ..., ge=1, description='Estimated tokens needed'
    ),
    db: AsyncSession = Depends(get_async_session),
    current_user: User = Depends(get_current_active_user),
) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail='Failed to check usage limits'
        )
...
async def get_monthly_report(
    ...
    try:
        ...
        if month == 12:
            ...
            month_end = datetime(year, month + 1, 1, tzinfo=UTC) - timedelta(
                seconds=1
            )

        service = AIBillingService(db)
        report = await service.calculate_usage_bill(
            organization_id, month_start, month_end
        )
        ...
        raise HTTPException(
            status_code=500, detail='Failed to generate monthly report'
        )
