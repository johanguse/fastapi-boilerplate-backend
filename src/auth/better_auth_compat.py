...
async def oauth_callback(
    ...
    try:
        ...
        user_info = await oauth_providers.get_user_info(provider, token["access_token"])
        ...
        if user:
            ...
            # Create new user through UserManager
            user_manager: UserManager = get_user_manager()
            user_create = UserCreate(
                email=user_info["email"],
                password=secrets.token_urlsafe(32),  # Random password
                name=user_info.get("name", ""),
                is_verified=user_info.get("email_verified", True)  # OAuth emails are generally verified
            )
        ...
        if hasattr(request, 'session') and "oauth_redirect_url" in request.session:
...
async def unlink_oauth_account(
    ...
    if (not hasattr(current_user, 'hashed_password') or not current_user.hashed_password) and current_user.oauth_provider == provider:
        raise HTTPException(status_code=400, detail={
    ...
    return {
        "providers": [
            ...
            },
            {
                "id": "apple",
                "name": "Apple",
                "enabled": bool(settings.APPLE_CLIENT_ID and settings.APPLE_CLIENT_SECRET)
            }
        ]
    }