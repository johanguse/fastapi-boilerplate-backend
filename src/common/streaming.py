...
class JSONStreamer:
    ...
    @staticmethod
    def stream_json_array(items: AsyncGenerator[Dict[str, Any], None]) -> AsyncGenerator[str, None]:
        """
        ...
        async def generate():
            ...
            for key, value in obj.items():
                if key != stream_key:
                    if object_start != '{':
                        object_start += ','
                    object_start += f'"{key}": {json.dumps(value, default=str)}'
...
class CSVStreamer:
    ...
    def stream_csv(
        headers: List[str], 
        rows: AsyncGenerator[List[Any], None]
    ) -> AsyncGenerator[str, None]:
        """
