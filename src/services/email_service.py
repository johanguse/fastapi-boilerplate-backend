...
class EmailService:
    ...
    async def send_verification_email(self, email: str, token: str, name: Optional[str] = None) -> bool:
        """
    ...
    async def send_forgot_password_email(self, email: str, token: str, name: Optional[str] = None) -> bool:
        """
        ...
        try:
            ...
            response = resend.emails.send(params)
            logger.info(f"Password reset email sent to {email}, ID: {response.get('id')}")
            return True
