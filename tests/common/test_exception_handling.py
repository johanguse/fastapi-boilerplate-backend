...
class TestExceptionContexts:
    ...
    def test_permission_error_in_project_context(self):
        ...
        assert any(
            word in error_str
            for word in ['member', 'organization', 'permission']
        )
    ...
    def test_validation_error_in_form_context(self):
        ...
        error_str = str(err).lower()
        assert any(
            word in error_str for word in ['validation', 'invalid', 'format']
        )
...
class TestTranslationBehavior:
    ...
    def test_custom_translation_key(self):
        ...
        err = NotFoundError(
            detail='Default message', translation_key='error.not_found'
        )
