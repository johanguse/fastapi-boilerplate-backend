...
async def test_upload_file_unsupported_type(authenticated_client, monkeypatch):
    ...
    monkeypatch.setattr(settings, "ALLOWED_FILE_TYPES", ["image/png"], raising=False)
...
async def test_upload_file_storage_failure(authenticated_client, monkeypatch):
    ...
    monkeypatch.setattr(settings, "ALLOWED_FILE_TYPES", ["text/plain"], raising=False)
    ...
    monkeypatch.setattr(upload_routes, "upload_file_to_r2", fake_upload_file_to_r2)
...
async def test_delete_file_failure(authenticated_client, monkeypatch):
    ...
    monkeypatch.setattr(upload_routes, "delete_file_from_r2", fake_delete_file_from_r2)
