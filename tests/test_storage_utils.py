...
from src.utils import storage as storage_utils  # noqa: E402
...
async def test_upload_file_to_r2_success(monkeypatch):
    monkeypatch.setattr(
        storage_utils, 'get_r2_client', lambda: DummyClient(should_fail=False)
    )
    url = await storage_utils.upload_file_to_r2(
        b'data', 'key.txt', 'text/plain'
    )
    assert url
    assert url.endswith('/key.txt')


@pytest.mark.asyncio
async def test_upload_file_to_r2_failure(monkeypatch):
    monkeypatch.setattr(
        storage_utils, 'get_r2_client', lambda: DummyClient(should_fail=True)
    )
    url = await storage_utils.upload_file_to_r2(
        b'data', 'key.txt', 'text/plain'
    )
