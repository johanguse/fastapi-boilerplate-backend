...
async def test_password_hashing_and_verification(async_session):
    ...
    is_valid, updated_hash = user_manager.password_helper.verify_and_update(password, hashed)
    assert is_valid is True
    assert updated_hash is None  # No update needed for fresh hash
    
    # Test wrong password
    is_valid, updated_hash = user_manager.password_helper.verify_and_update("wrong_password", hashed)
...
async def test_password_helper_consistency(async_session):
    ...
    is_valid1, _ = user_manager.password_helper.verify_and_update(password, hash1)
    is_valid2, _ = user_manager.password_helper.verify_and_update(password, hash2)
...
async def test_bcrypt_compatibility(async_session):
    ...
    bcrypt_hash = "$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewdBPj8xq/2vV3za"
    
    # Test if the password helper can still verify bcrypt hashes
    is_valid, updated_hash = user_manager.password_helper.verify_and_update("admin123", bcrypt_hash)
    ...
    if not is_valid:
        ...
        pytest.skip("Bcrypt compatibility not available - migration needed for existing users")
    else:
        assert is_valid is True
...
async def test_empty_password_handling(async_session):
    ...
    is_valid, _ = user_manager.password_helper.verify_and_update("", empty_hash)
    assert is_valid is True
    
    # Test that non-empty password doesn't verify against empty hash
    is_valid, _ = user_manager.password_helper.verify_and_update("not_empty", empty_hash)
    assert is_valid is False