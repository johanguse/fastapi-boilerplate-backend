...
async def test_create_project_membership_required(monkeypatch):
    ...
    monkeypatch.setattr(
        proj_service, 'is_org_member', AsyncMock(return_value=False)
    )

    with pytest.raises(Exception, match='member') as e:
...
async def test_create_project_plan_limits(monkeypatch):
    ...
    monkeypatch.setattr(
        proj_service, 'is_org_member', AsyncMock(return_value=True)
    )
    ...
    monkeypatch.setattr(
        proj_service,
        'get_organization',
        AsyncMock(return_value=OrgObj('starter', 1)),
    )
    ...
    monkeypatch.setattr(
        proj_service,
        'get_organization',
        AsyncMock(return_value=OrgObj('pro', proj_service.MAX_PRO_PROJECTS)),
    )
    ...
    monkeypatch.setattr(
        ...
        'get_organization',
        AsyncMock(
            return_value=OrgObj('business', proj_service.MAX_BUSINESS_PROJECTS)
        ),
    )
...
async def test_get_project_and_update_and_delete(monkeypatch):
    ...
    upd = await proj_service.update_project(
        db2, 9, SimpleNamespace(model_dump=lambda **_: {'name': 'Y'}), user
    )
    assert upd.name == 'Y'
...
async def test_create_project_success(monkeypatch):
    ...
    monkeypatch.setattr(
        proj_service, 'get_organization', AsyncMock(return_value=org)
    )

    project_data = SimpleNamespace(
        name='Test Project', description='Test description', organization_id=1
    )
    ...
    assert (
        db.commits >= 2
    )  # One for project creation, one for updating org active_projects
...
async def test_get_project_not_found(monkeypatch):
    ...
    with pytest.raises(Exception, match='not found') as e:
